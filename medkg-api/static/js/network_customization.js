...
document.getElementById('network_customization').addEventListener('click', function() {
    const panel = document.getElementById('network_customization_panel');
    panel.classList.toggle('show-panel');
});

document.getElementById('zoom-in').addEventListener('click', function() {
	cy.zoom({
	  level: cy.zoom() * 1.2,  // Increase zoom by 20%
  ...
  document.getElementById('zoom-out').addEventListener('click', function() {
	cy.zoom({
	  level: cy.zoom() * 0.8,  // Decrease zoom by 20%
	  renderedPosition: { x: cy.width() / 2, y: cy.height() / 2 }
	});
  });
...
$('#edgelabel_on_off_btn').click(function() {
	
	if($('#edgelabel_on_off_btn').hasClass('btn-secondary')){

		cy.$('edge').addClass("nolabel");

		$('#edgelabel_on_off_btn').removeClass('btn-secondary');
	}else{
		cy.$('edge').removeClass("nolabel");
		$('#edgelabel_on_off_btn').addClass('btn-secondary');
	}
});
